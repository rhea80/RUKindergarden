...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
        ...
        for (int i = 0; i < numberOfStudents; i++){
            ...
            if (studentsInLine.getNext() == null && studentsInLine.getStudent() == null )
            {
                studentsInLine.setStudent(newStudent);
            }
            else{
                SNode nextNode = new SNode (newStudent, studentsInLine);
    ...
    public void seatStudents () {
        int rows = seatingLocation.length;
        int columns = seatingLocation[0].length;
    ...
    public void insertMusicalChairs () {
        ...
        int rows = studentsSitting.length;
        int columns = studentsSitting[0].length;
        musicalChairs = new SNode();

        for(int i = 0; i < rows; i++){
            for(int j = 0; j < columns; j++){
                if(studentsSitting[i][j] != null){
                    if (musicalChairs.getNext() == null && musicalChairs.getStudent() == null )
                    {
                        musicalChairs.setStudent(studentsSitting[i][j]);
                        seatingLocation[i][j] = false; 
                    }
                    else {
                        SNode nextNode = new SNode (studentsSitting[i][j], musicalChairs);
                        musicalChairs.setNext(nextNode);
    ...
    public void seatMusicalChairsWinner () {
        ...
        if (musicalChairs.getNext() == musicalChairs){
            for (int i = 0; i < seatingLocation.length && !stop; i++){
                for (int j = 0; j < seatingLocation[i].length && !stop; j++){
                    if (seatingLocation[i][j] == true){
